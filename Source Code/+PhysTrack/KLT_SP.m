function [trajectories, vr2o_new] = KLT_SP(vr2o, objs, previewDownSample, nouserquestion)
%KLTMPT KLT MultiPointTracker
%   This tracker is based on KLT algorithm. It uses the in-built KLT
%   tracker to track a single objects at a time
if nargin <= 3
...
if nargin == 2 
    ...
    global klt_tObs_00 klt_vr2o_00 klt_objs_00
    klt_vr2o_00 = vr2o;
    klt_objs_00 = objs;
    ...
    while ff < klt_vr2o_00.TotalFrames
        ...
        for kk = 1: size(objs, 1)
            inS = num2str(kk);
            [tPoints, validity] = step(tracker1, frame);
            if rem(ff, previewDownSample) == 0
                out = insertMarker(out, tPoints(validity, :), 'o', 'Size', 5, 'Color', PhysTrack.GetColor(1));
            end
            ...
            if resetObjs
                ...
                if  size(klt_trackPoints_00_1, 1) > 0
                    if size(tPoints, 1) > size(klt_trackPoints_00_1, 1)
                        xmax = size(tPoints, 1);
                        ymax = size(klt_trackPoints_00_1, 2);
                        zmax = size(klt_trackPoints_00_1, 3);
                        klt_trackPoints_00_1(xmax, ymax, zmax) = 0;
                        klt_PointsValidity_00_1(xmax, zmax) = 0;
                    elseif size(tPoints, 1) < size(klt_trackPoints_00_1, 1)
                        xmax = size(klt_trackPoints_00_1, 1);
                        tPoints(xmax, 2) = 0;
                        validity(xmax) = 0;
                    end
                end
                ind = ff;
                klt_trackPoints_00_1(:,:, ind) = tPoints;
                klt_PointsValidity_00_1(:,ind) = validity;
            end
        end
        if framesDone >= 20
